        ...
        if (pmm.getType() == MemoryManagerType.PAGING) {
            ...
            if (pa == null) {
                ...
                if (pageVictim == -1) { // If no victim was found because there are still frames available
                    ...
                    frameVictim = pmmp.getFrameMemoryAddressFromLogicalMemoryAddress(pageVictim); // Find the frame
                                                                                                  // number in memory of
                                                                                                  // the victim page
                    frameVictimInSwap = pmmp.getVFrameMemoryAddressFromLogicalMemoryAddress(pageVictim); // Find the
                                                                                                         // frame number
                                                                                                         // in Swap
                                                                                                         // memory of
                                                                                                         // the victim
                                                                                                         // page
                    System.out.println("[VM][" + OS.PVMM + "] Victim selected - page: " + pageVictim + " in frame: "
                            + frameVictim);
                }

                int pageToLoad = la.getDivision(); // Get the pageID of the desired page
                int frameToLoadInSwap = pmmp.getVFrameMemoryAddressFromLogicalMemoryAddress(pageToLoad); // Find the
                                                                                                         // frame number
                                                                                                         // in Swap
                                                                                                         // memory of
                                                                                                         // the desired
                ...
                MemoryPageExchange mpe = new MemoryPageExchange(pageVictim, frameVictimInSwap, frameVictim, pageToLoad,
                        frameToLoadInSwap);

                if (pageVictim != -1) { // If there was a page identified to leave memory, then it may have to be sent
                                        // to swap memory
                    if (pmmp.isPageDirty(pageVictim)) { // If the page is dirty, then it must be updated in the swap
                                                        // memory
                        mpe.setFullExchange(true); // This is a full exchange, so the frame does not have to be freed
                        getOS().interrupt(InterruptType.STORE_PAGE, pmmp.getProcess(), mpe); // Send the frame in memory
                                                                                             // of page la to swap
                                                                                             // memory. All the
                ...
                getOS().interrupt(InterruptType.LOAD_PAGE, pmmp.getProcess(), mpe); // Load the frame in swap of page la
                                                                                    // to real memory
