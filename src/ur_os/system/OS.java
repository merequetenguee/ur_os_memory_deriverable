...
public class OS {
    ...
    public void interrupt(InterruptType t, Process p, Object i) {
        ...
        switch (t) {
            ...
            case MEMORY_DONE: // It is assumed that the process in Memory is done and it has been removed from
                              // the queue
                rq.addProcess(p);
                break;
            ...
            case STORE_PAGE:
                ...
                if (!mpe.isFullExchange()) { // If it is a fullExchange, then the frame will be used to load another
                                             // page
            ...
            case CONTIGUOUS:
                if (VIRTUAL_MEMORY_MODE_ON) {
                    ...
                    if (p.getSize() == 0) {
                        ...
                        pmm = new PMM_Contiguous(p, getVMemorySlot(p.getSize()), getMemorySlot(p.getSize()),
                                this.lazySwap);
                }
                 ...
                 * if(!lazySwap){
                 * PMM_Contiguous pmmc = (PMM_Contiguous)pmm;
                 * pmmc.setMemorySlot(getMemorySlot(p.getSize())); //get free slot and assign it
                 * to the process and store the process in memory.
                 * }
                 */
                break;
        }
